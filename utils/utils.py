...
def display_bertscores(r):
    return list(zip(['bert_p','bert_r','bert_f1'],r))
...
def meteor_from_multiple_refs(pred, references):
    pred_tk = word_tokenize(pred)
    references_tk = [word_tokenize(r) for r in references]
    return meteor_score(references_tk, pred_tk)

def bertscore_from_multiple_refs(pred, references):
    # Compute BERTScore
    P, R, F1 = map(np.array, bert_score.score([pred] * len(references), references, lang='en'))
    #P, R, F1 = map(np.array, zip(*[bert_score.score([pred], [r], lang='en') for r in references]))

    # Get the index of the reference with the max F1 score
    max_index = F1.argmax().item()

    # Get the corresponding precision, recall, and F1 score
    max_precision = P[max_index].item()
    max_recall = R[max_index].item()
    max_f1_score = F1[max_index].item()
