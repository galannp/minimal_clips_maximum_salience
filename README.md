...
2. Cutting the Video in 20 seconds video clips
python -m prepare_data.video_segmentation

## Video Clip Selection
We set the number of selected clips K=50 in what follows (can be replaced by other values).
...
python -m clip_selection.silent.caption_silent_clips --captioning_model gemini --api_key `YOUR_GOOGLE_API_KEY`
...
<p align="center">
<img src="./figs/plot_25.png" width="768"/>
</p>

<p align="center">
<img src="./figs/plot_50.png" width="768"/>
</p>

<p align="center">
<img src="./figs/plot_75.png" width="768"/>
</p>


## Screenplay Summarization
1. Build the Screenplays
For the clip_selection parameter you can choose between `ours`, `random` or `silent`.
...
This step generate a summary from the screenplay built in the previous step.
The path to the screenplay is given by `path` in parameter of the python command.
Choose the summarization model by setting `version` ('gemini-2.5-flash', 'gemini-1.5-flash' or 'qwen2-72b-instruct').
You must set a valid GOOGLE API key `api_key` if you choose a Gemini model.
In all our experiments we set the summary length to 1000 words.
...
## Screenplay of Reference Clips (Optional)
...
## Evaluation
1. Evaluation on Traditional Metrics
This step evaluates the generated summaries on two traditional summarization metrics: ROUGE and METEOR.
In our metrics computations, we always truncate the summaries to 1000 words.
The path to the generated summaries is given by `expname` in parameter of the following python command:
...
This step evaluates the generated summaries on MFactSum, a metric for multimodal summarization.
In our metrics computations, we always truncate the summaries to 1000 words.
The path to the generated summaries is given by `expname` in parameter of the following python command:

python -m summ.eval.MFactSum_fact_eval --truncate_lim 1000 --expname `summmary_dir` --api_key `YOUR_GOOGLE_API_KEY`
